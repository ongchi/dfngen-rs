...
use super::UserDefinedRectByCoord;
...
pub fn insert_user_rects_by_coord(
    ...
    poly_def: &UserDefinedRectByCoord,
) {
    let family_id = -2;
    ...
    for (i, mut new_poly) in new_polys.into_iter().enumerate() {
        if domain_truncation(h, eps, &mut new_poly, domain_size) {
            ...
            warn!("User Rectangle (defined by coordinates) {} was rejected for being outside the defined domain.", i + 1);
            pstats
